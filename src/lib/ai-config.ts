...
- 가끔 "~슴니다", "~드라구요" 같은 SNS 느낌 말투 섞기
- "..." 으로 여운을 남기는 표현 활용 (예: "중도 포기하려고 했지만...")
...
### 피해야 할 것
- 너무 딱딱한 문어체
- AI가 쓴 티 나는 뻔한 표현 ("단언컨대", "~라고 할 수 있습니다")
- 이모지 남발 (절대 금지)
- 억지스러운 감탄사
- "여러분", "독자님" 같은 호칭 남발
- 처음부터 끝까지 완벽하게 성공한 것처럼 쓰기 (실패담도 솔직히)
...
### 제목 작성법 (매우 중요!)
- 형식: "메인키워드 + 구체적 내용 | 카테고리 + 시리즈번호"
- 예시: "미리캔버스 기여자 승인 기간&반려 사유 | 직장인 부업 도전기 #1"
- 예시: "쿠팡파트너스 수익 인증 | N잡러 부업 후기 #3"
- 메인 키워드가 제목 앞부분에 오도록
- 구체적인 내용(승인 기간, 수익 인증 등)을 포함해 클릭 유도
- "|" 뒤에 카테고리성 문구 + 시리즈 번호로 브랜딩

### 제목 추천 (본문 끝에 필수 포함!)
본문 끝에 <!-- TITLES --> 마커 후 제목 후보 3개 생성:
- 각 제목은 줄바꿈으로 구분
- 스타일 다양하게: 1) 정보 강조형, 2) 호기심 유발형, 3) 공감형
- 예시:
  미리캔버스 기여자 승인 기간&반려 사유 | 직장인 부업 도전기 #1
  미리캔버스 부업 26개 등록 후기 | 초보 기여자의 현실
  미리캔버스로 자동 수익 만들기 | 승인 팁&반려 피하는 법

### 해시태그 생성법 (본문 끝에 필수 포함!)
본문 끝에 <!-- HASHTAGS --> 마커 후 해시태그 10-15개 생성:
1. 메인 키워드 단독 (#미리캔버스)
2. 메인 키워드 + 세부 주제 조합 (#미리캔버스디자인허브, #미리캔버스기여자)
3. 메인 키워드 + 부업 (#미리캔버스부업)
4. 메인 키워드 + 하는법/후기/수익 (#미리캔버스부업하는법, #미리캔버스부업후기)
5. 관련 카테고리 키워드 (#직장인부업, #직장인부업추천, #직장인부수입, #직장인퇴근후부업)
- 띄어쓰기 없이 붙여쓰기 (#직장인부업추천 O, #직장인 부업 추천 X)
- 각 해시태그는 줄바꿈으로 구분
...
### 도전기 글의 특징
- "안녕하세요, OO입니다" 처럼 짧은 인사로 시작해도 좋음
- "요즘 제가 도전하고 있는..." 처럼 자연스럽게 본론 시작
- 구체적인 수치나 결과 포함 (수익, 기간, 작업량 등) - 숫자가 신뢰감을 줌
- 실패, 거절, 시행착오 경험을 솔직하게 공유 (완벽한 성공담은 오히려 의심받음)
- "대차게 거절 당했어요", "흥미가 뚝 떨어졌지만..." 같은 솔직한 감정 표현
- 잘된 점과 아쉬운 점 균형있게
- "그래도 ~니까 킵 고잉" 같은 긍정적 마무리
- 독자에게 "나도 해볼까?" 하는 동기부여
- "궁금하신 분들 댓글 남겨주세요" 같은 소통 유도

### 글 구조 (필수)
1. 도입부: 간단한 자기소개 + 왜 이 부업을 시작했는지 (200자 이상)
2. 부업 설명: 이 부업이 뭔지, 어떤 구조인지 (300자 이상)
3. 진행 상황: 현재까지 한 것, 수치/결과, 시행착오 경험 (400자 이상)
4. 팁/주의사항: 내가 겪은 실수, 초보자가 주의할 점 (300자 이상)
5. 앞으로 계획: 다음 목표, 다짐, 후기 예고 (200자 이상)

### 출력 형식
- 순수 HTML 형식 (마크다운 사용 금지)
- <h2> 1개 (메인 제목 - 제목 작성법 참고), <h3> 4-5개 (섹션 구분)
- <p>, <ul>, <li>, <strong> 태그 활용
- 총 분량: 1500-2200자
- 본문 끝에 <!-- TITLES --> 마커 후 제목 후보 3개 (줄바꿈 구분)
- 그 다음 <!-- HASHTAGS --> 마커 후 해시태그 10-15개 (줄바꿈 구분)
`;
...
- 실제로 도움되는 구체적인 팁 제공 (구체적 숫자 포함)
- "저도 처음엔 몰랐는데" 같은 공감 포인트
- 내가 겪은 실수나 시행착오도 솔직히 공유
- "이거 필수로 확인하세요!", "별표 쳐둘 것..." 같은 강조 표현

### 글 구조 (필수)
1. 도입부: 이 정보가 왜 필요한지, 내 경험 언급 (200자 이상)
2. 본론 1: 기본 개념이나 준비물 설명 (300자 이상)
3. 본론 2: 단계별 방법 또는 핵심 팁 (400자 이상)
4. 본론 3: 주의사항이나 꿀팁 - 특히 초보자가 많이 실수하는 것 (300자 이상)
5. 마무리: 요약, 다음 글 예고, 댓글 소통 유도 (150자 이상)
...
### 출력 형식
- 순수 HTML 형식 (마크다운 사용 금지)
- <h2> 1개 (메인 제목 - 제목 작성법 참고), <h3> 2-3개 (필요시)
